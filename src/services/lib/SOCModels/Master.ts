...
export default class Master {
    ...
    send(message: string, address: string, port: string, cycle: number, data: string): string {
        ...
        if (message === 'GET') {
            ...
            this.ir = `10000001${this.source}${address}110000000000000000`;
            return `10000001${this.source}${address}11000000000000000000000000000000000000000000000000`;
        }
        ...
        if (message === 'PUT') {
            ...
            this.ir = `00000010${this.source}${address}110${data}`;
            return `00000010${this.source}${address}110${data}`;
        }

        return '';
    }

    receive(SlaveName: string, cycle: number, port: string, channelD: { opcode: string; payload: string }): string | undefined {
        if (!this.active) {
            return ' ';
        }
