...
export default class Slave {
    ...
    send(cycle: number, port: string, message: string, data: string): string {
        ...
        if (message === 'AccessAck') {
            this.AccessAck += 1;
            this.ir = ('000' + '00' + '01' + this.source + '000' + '0' + '0000000000000000' + '0' + '0').padEnd(36, '0');
    ...
    receive(cycle: number, port: string, ChannelA: { source: string; opcode: string; data: string; address: string }): [string, string] {
        if (!this.active) {
            return ['', ''];
        }
