        ...
        while (this.Processor.pc< Object.values(this.Processor.Instruction_memory).length * 4) {
            ...
            if (message== "PUT") {
                ...
                if (dec('0'+address) <10 && 0<= dec('0'+address)) {
                    ...
                    const [di2s, ai2s] = this.Memory.slaveDataMemory.receive(this.cycle, 'Port_out[1]', doutChA)
                    this.Memory.DataMemory[ai2s]= di2s
                
                    this.cycle+= 1
                    const doutChD= this.Memory.slaveDataMemory.send(this.cycle, 'Port_in[1]', 'AccessAck', '')
                ...
                if (dec('0'+address) <20 && 10<= dec('0'+address)) {
                    ...
                    const [di2s, ai2s] = this.Memory.slaveIOMemory.receive(this.cycle, 'Port_out[2]', doutChA)
                    this.Memory.IOMemory[ai2s]= di2s
                
                    this.cycle+= 1
                    const doutChD= this.Memory.slaveIOMemory.send(this.cycle, 'Port_in[2]', 'AccessAck', '')
            ...
            if (message== "GET") {
                ...
                if (dec('0'+address) <10 && 0<= dec('0'+address)) {
                    const doutChA= this.Bus.Port_out(1)
                    const [, ai2s] = this.Memory.slaveDataMemory.receive(this.cycle, 'Port_out[1]', doutChA)
                    //const di2s= this.Memory.DataMemory[ai2s]
                    const di2s= this.Memory.DataMemory[ai2s]
                    this.cycle+= 1
                    const doutChD= this.Memory.slaveDataMemory.send(this.cycle, 'Port_in[1]', 'AccessAckData', di2s)
                ...
                if (dec('0'+address) <20 && 10<= dec('0'+address)) {
                    const doutChA= this.Bus.Port_out(2)
                    const [, ai2s] = this.Memory.slaveIOMemory.receive(this.cycle, 'Port_out[2]', doutChA)
                    //const di2s= this.Memory.DataMemory[ai2s]
                    const di2s= this.Memory.IOMemory[ai2s]
                    this.cycle+= 1
                    const doutChD= this.Memory.slaveIOMemory.send(this.cycle, 'Port_in[2]', 'AccessAckData', di2s)
                    console.log('channelD',doutChD)
                    this.Bus.Port_in_CD(doutChD, 2, this.cycle)
                    this.Bus.TransmitChannelD()
                    //console.log('wrietData',this.Bus.Port_out(0).payload)
