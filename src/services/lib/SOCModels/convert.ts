...
export function handleInstructionMemory(instructionMemory: { [key: string]: string }): { [key: string]: string } {
    ...
    for (const i in instructionMemory) {
        ...
        instructionArr['0x' + dec('0' + i).toString(16).padStart(8, '0')] = '0x' + instructions;
    }
...
export function handleRegister(register: { [key: string]: string }): { [key: string]: string } {
    const re = ['zero', 'ra', 'sp', 'gp', 'tp', 't0', 't1', 't2', 's0/fp', 's1', 'a0', 'a1', 'a2', 'a3', 'a4', 'a5', 'a6', 'a7', 's2', 's3', 's4', 's5', 's6', 's7', 's8', 's9', 's10', 's11', 't3', 't4', 't5', 't6', '0'];
...
export function handleDataMemory(dataMemory: { [key: string]: string }): string[] {
    ...
    for (const i in dataMemory) {
        dataList.push(convertHexToDec('0x' + dec('0' + i).toString(16).padStart(8, '0')));
    }
    ...
    for (let i = 0; i < dataList.length; i++) {
        ...
        const hexKey = dec('0' + key).toString(16).padStart(8, '0');
        const hexValue = (value < 0 ? (1 << 32) + value : value).toString(16).padStart(8, '0');
...
export function rd(instruction: string): string {
    ...
    if (['0110011', '0010011', '0000011', '0110111', '0010111', '1101111', '1100111'].includes(opcode)) {
